...
func GetTaskProviderVerifyTasksFromAssets(assets Assets) []VerifyTaskInfo {
	var verifyTaskInfos []VerifyTaskInfo
	for _, assetType := range slices.Sorted(maps.Keys(assets.assets)) {
		assetsForType := assets.assets[assetType]

		// currently, only dister asset type is supported as task-provider
		if assetType != Dister {
			continue
		}

		for _, currAsset := range assetsForType {
