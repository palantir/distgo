...
func (f *disterFactoryImpl) NewDister(typeName string, cfgYMLBytes []byte) (distgo.DisterWithConfig, error) {
	...
	createdDister, err := creatorFn(cfgYMLBytes)
	if err != nil {
		return nil, err
	}
	return distgo.NewDisterWithConfig(createdDister, cfgYMLBytes), nil
}
